...
def webhook(request):
    try:
        ...
        if text == "/start":
            ...
            send_message_text(chat_id, "Hi! Welcome to the Road Rating Bot.")
            send_message_text(chat_id, "To rate a road, type /start \nTo cancel the current operation, type /cancel \nTo see your past ratings, type /past \n\nTo see this message again, type /help")                
            send_message_text(chat_id, "Enter the name of the road you want to rate?")
            conv.step = "ask_road"
            conv.save()
        ...
        if text == "/past":
            ...
            if past_ratings.exists():
                send_message_text(chat_id, "📝 Your past ratings:")
                ...
                send_message_text(chat_id, "ℹ️ You haven’t rated any roads yet.")
            return JsonResponse({"ok": True})

        if text == "/cancel":
            if latest_conv and latest_conv.step != "complete":
                latest_conv.delete()
            send_message_text(chat_id, "Cancelled. To start over, type /start")
            return JsonResponse({"ok": True})

        if conv is None:
            # No active conversation, ask them to start
            send_message_text(chat_id, "Please type /start to begin rating a road.")
            return JsonResponse({"ok": True})

        if conv.step == "complete":
            send_message_text(chat_id, "Thanks! To start a new rating, type /start")                
            return JsonResponse({"ok": True})
        ...
        if conv.step == "ask_road":
            ...
            conv.save()
            send_message_text(chat_id, "Thanks! Now give me a rating (1-5):")

        elif conv.step == "ask_rating":
            if text not in ["1", "2", "3", "4", "5"]:
                send_message_text(chat_id, "⚠️ Please provide a valid rating between 1 and 5.")
                return JsonResponse({"ok": True})
            conv.rating = text
            conv.step = "ask_gps"
            conv.save()
            send_message_text(chat_id, "Thanks! Now please provide the GPS coordinates of the road:")
            ...
            if location:  # user sent attached location
                ...
                send_message_text(chat_id, "⚠️ Please provide GPS coordinates (or /skip).")
                return JsonResponse({"ok": True})
            
            conv.step = "ask_comments"
            conv.save()
            send_message_text(chat_id, "Got it! Please add any comments:")
            ...
            send_message_text(chat_id, "✅ Feedback submitted. Thank you!")
            send_message_text(chat_id, "Want to add another? Please type /start")
        
        else:
            send_message_text(chat_id, "⚠️ Seems like a wrong command.\n ")
            send_message_text(chat_id, "To rate a road, type /start \n To cancel the current operation, type /cancel \n To see your past ratings, type /past \n\nTo see this message again, type /help")                
