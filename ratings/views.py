# This file contains the views for the road rating application.
# It handles the logic for the HTTP requests and responses.
# It also contains the code for interacting with the Telegram bot API.
# The views are decorated with the @csrf_exempt decorator to allow cross-origin requests.
# The file also imports the necessary modules and models from the application.
# It logs the errors and debug messages using the logging module.
# The file is responsible for handling the media uploads and the road rating data.
import mimetypes
import boto3
from django.conf import settings
from django.http import JsonResponse
from .models import RoadMedia, RoadRating, UserConversation, TeleUser
import requests
import json
import os
import re
...
def webhook_widgets(request):
    ...
    if "photo" in message or "video" in message or "document" in message:
        ...
        if not road_id:
            send_message_markdown(
                chat_id, "⚠️ Please start a rating session first by typing /start and following the prompts.")
            return JsonResponse({"ok": False})
        if handle_media_upload(message, chat_id, session, road_id):
            ...
            send_message_markdown(
                chat_id, "⚠️ Could not upload media. Please try again from dashboard. Thanks.")
    ...
    if "text" in message:
        ...
        # Start rating
            ...
            send_message_markdown(
                chat_id, "Please enter the road name:", reply_markup=keyboard)
        ...
        # Skip location
            ...
            send_message_markdown(
                chat_id, "📎 Please upload any supporting media (photos, videos) you want to attach to this rating.", reply_markup=keyboard)
        ...
        # View past ratings
            ...
            send_message_markdown(
                chat_id, "⚠️ Unrecognized command. Please use the buttons to navigate. To start a new rating, type /start")
    ...
    # Handle location
        ...
        send_message_markdown(
            chat_id, "⚠️ Unrecognized input. Please use the buttons to navigate. To start a new rating, type /start")
...
def exiting_program(chat_id):
    ...
    send_message_markdown(
        chat_id, "👋 Thank you for using the Road Rating Bot! To start again, type /start", reply_markup=keyboard)
...
def show_dashboard_otp_logic(chat_id):
    ...
    logger.info(
        f"Generated OTP {secret_otp} and token {token} for chat_id {chat_id}")
    if set_otp_for_user(chat_id, secret_otp):
        logger.info(
            f"sending message to chat_id {chat_id} with token {token} and otp {secret_otp}")
        send_message_markdown(
            chat_id, f"To access the dashboard, go to {url} \n Password: {secret_otp}", parse_mode="HTML")
    else:
        send_message_markdown(
            chat_id, "⚠️ Unable to set OTP for your user. Please contact support.")
...
def add_rating_prompt(chat_id):
    ...
    send_message_markdown(
        chat_id, "⭐ Please rate the road (1–5):", reply_markup=keyboard)
...
def add_comment_prompt(chat_id):
    ...
    send_message_markdown(
        chat_id, "Would you like to add a comment?", reply_markup=keyboard)
...
def add_location_prompt(chat_id):
    ...
    send_message_markdown(
        chat_id, "📍 Please share the location:", reply_markup=keyboard)


def past_rating(chat_id):
    RECENT_RATINGS = 10
    past_ratings = RoadRating.objects.filter(
        fk_road_id__fk_chat_id__chat_id=chat_id).order_by("-created_at")[:RECENT_RATINGS]
    logger.info(
        f"Found {past_ratings.count()} past ratings for chat_id {chat_id}, showing up to {RECENT_RATINGS}")
    if past_ratings.exists():
        send_message_markdown(
            chat_id, f"📝 Showing Your past {RECENT_RATINGS} ratings:")
        ...
        send_message_markdown(
            chat_id, "ℹ️ It seems you haven’t rated any roads yet.")
...
def create_road_rating_and_conversation(chat_id):
    ...
    if not session:
        send_message_markdown(
            chat_id, "⚠️ Session expired or invalid. Please start again with /start")
        return
    logger.info(
        f"💾 Creating road rating and conversation for chat_id {chat_id}: {session}")
    ...
    t_user = session.get(
        "tuser") or create_teleuser_if_not_exists(chat_id=chat_id)
    UserConversation.objects.create(
        fk_chat_id=t_user,
        fk_road_id=feedback,
    )
    logger.info(
        f"Created RoadRating {feedback.id} and UserConversation for chat_id {chat_id}")
...
def save_rating(chat_id):
    send_message_markdown(
        chat_id, "✅ Your road rating has been saved! Thank you 🙏")
...
def add_media_prompt(chat_id):
    ...
    send_message_markdown(
        chat_id, "Would you like to add any supporting media (photos, videos)?", reply_markup=keyboard)
...
def rate_road(chat_id):
    ...
    send_message_markdown(
        chat_id, "👋 Welcome to Road Rating Bot!", reply_markup=keyboard)
...
def want_to_continue(chat_id):
    ...
    send_message_markdown(
        chat_id, "👋 Do you want to rate more roads?", reply_markup=keyboard)
...
def create_teleuser_if_not_exists(chat_id, first_name=None, last_name=None, username=None, language_code=None, is_bot=False):
    ...
    if user_created:
        # hash properly the password... password is currently hardcoded
    ...
    if tele_user.user is None and user:
        tele_user.user = user
        logger.info(
            f"Linking TeleUser {tele_user.chat_id} to User {user.username}")
        tele_user.save()
        logger.info(
            f"Linked TeleUser {tele_user.chat_id} to User {user.username}")

    if created or user_created:
        logger.info(
            f"Created new TeleUser: {tele_user} (linked to User: {user.username})")
...
def handle_media_upload(message, chat_id, session, road_id):
    ...
    try:
        ...
        if "photo" in message:
            ...
            orig_filename = message["document"].get(
                "file_name") or f"{file_id}"
            content_type = (
            ...
            media_type = "doc"
            extension = os.path.splitext(orig_filename)[
                1] or mimetypes.guess_extension(content_type) or ""
        ...
        logger.info(
            f"Saved RoadMedia {road_media.id} for RoadRating {road_id}")
